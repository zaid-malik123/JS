...
describe('uploadFileInternal', () => {
    ...
    test('correctly builds URL with multimodal flag set to true', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?multimodal=true', expect.objectContaining({
            method: 'POST',
            body: expect.any(FormData),
            headers: expect.objectContaining({
                'Api-Key': 'test-api-key',
    ...
    test('correctly builds URL with multimodal flag set to false', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?multimodal=false', expect.objectContaining({
            method: 'POST',
            body: expect.any(FormData),
            headers: expect.objectContaining({
                'Api-Key': 'test-api-key',
    ...
    test('correctly builds URL with both metadata and multimodal', async () => {
        ...
        const encodedMetadata = encodeURIComponent(JSON.stringify(metadata));
        expect(mockFetch).toHaveBeenCalledWith(`https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?metadata=${encodedMetadata}&multimodal=true`, expect.objectContaining({
            method: 'POST',
            body: expect.any(FormData),
            headers: expect.objectContaining({
                'Api-Key': 'test-api-key',
